...
4. metric: 정렬 기준 컬럼. 질문이 특정 스탯을 요구하지 않으면(예: 소속팀/국적을 묻는 후속 질문) NONE.
5. sort_direction: "가장 많은/높은"→desc, "가장 적은/낮은"→asc. 기본 desc.
6. 복합조건 질문 처리 ("A 이상이면서 B도 많은/높은 X", "A와 B가 모두 높은 X" 등 조건이 2개 이상):
...
7. top_n: use_previous_candidates=false인 새 질문(단발/복합형)은 문장이 문법적으로 단수형("~는?")
...
8. use_previous_candidates=true일 때 metric 판단이 특히 중요하다. 두 유형을 구분해:
...
RESPONSE_SCHEMA = {
    ...
    "json_schema": {
        ...
        "schema": {
            ...
            "required": [
                "is_stat_query", "use_previous_candidates", "position",

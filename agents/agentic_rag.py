...
2. 특정 선수 이름을 콕 집어 물으면 player_lookup을 먼저 써.
3. 후속 질문에서 "그 중에서"처럼 직전 후보 집합 안에서 다시 걸러야 하면, 대화 기록에 남아있는
   직전 stat_query 결과의 선수 이름들을 네가 직접 골라서 stat_query의 player_names 인자로 넘겨
   그 후보 집합 안에서만 다시 필터/정렬해 (position은 이 경우 무시됨).
...
- 후속 질문 처리: "그 중 X가 많은/적은 선수는?"(그룹 내 재선별)은 X를 metric으로 쓰고
  sort_direction을 맞춰라(많은/높은→desc, 적은/낮은→asc), filters는 비워둬라. 반면 "그 선수 ~은?"
  (단수로 이미 정해진 1명을 다시 가리키며 설명·계산을 요구, 특히 목록에 없는 파생 지표를 물을 때)은
  metric="NONE"으로 두고 player_names로 이전 후보를 그대로 유지한 채 top_n=1만 적용해 — 새로
  재정렬하지 마라.
